...
def get_mrn_workflow_action_url(action, doc, user):
    apply_action_method = "/api/method/vcm.erpnext_vcm.overrides.mr_alm.mrworkflow_action.apply_action"
...
def get_approval_link(doc, user, allowed_options):
    if "L1 Approve" in allowed_options:
        return get_mrn_workflow_action_url(action="L1 Approve", doc=doc, user=user)
    if "L2 Approve" in allowed_options:
        return get_mrn_workflow_action_url(action="L2 Approve", doc=doc, user=user)
    if "Final Approve" in allowed_options:
        return get_mrn_workflow_action_url(action="Final Approve", doc=doc, user=user)
    else:
        frappe.throw(
            "Next ALM User is not allowed to approve the MRN Document. Please ask for permission."
        )


def get_rejection_link(doc, user):
    return get_mrn_workflow_action_url("Reject", doc, user)
