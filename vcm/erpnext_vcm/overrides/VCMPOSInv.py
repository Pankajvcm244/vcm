...
class VCMPOSInv(POSInvoice):
    ...
    def on_submit(self):
        ...
        if (self.pos_profile == "Krishna Prasadam"):  
            if self.customer == 'FOC':
                if not self.custom_foc__devotee_name:
    ...
    def send_KPFOC_email(self):
        # check if doc is ready to submit and not in draft (state 0)
        if self.docstatus == 1:   
            recipient_doc = self.custom_foc__devotee_email
            ...
            if (self.pos_profile == 'Krishna Prasadam'): 
                message = (
                    f'Hare Krishna {self.custom_foc__devotee_name},<br><br>'
