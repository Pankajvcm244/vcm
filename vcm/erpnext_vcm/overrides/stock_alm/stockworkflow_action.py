...
def get_stock_workflow_action_url(action, doc, user):
    apply_action_method = "/api/method/vcm.erpnext_vcm.overrides.stock_alm.stockworkflow_action.apply_action"
...
def get_approval_link(doc, user, allowed_options):
    if "Recommend" in allowed_options:
        return get_stock_workflow_action_url(action="Recommend", doc=doc, user=user)
    if "First Approve" in allowed_options:
        return get_stock_workflow_action_url(action="First Approve", doc=doc, user=user)
    if "Final Approve" in allowed_options:
        return get_stock_workflow_action_url(action="Final Approve", doc=doc, user=user)
    else:
        frappe.throw(
...
def get_rejection_link(doc, user):
    return get_stock_workflow_action_url("Reject", doc, user)
