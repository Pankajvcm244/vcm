...
frappe.ui.form.on("VCM PR Label Print", {
    refresh(frm) {
      frm.add_custom_button(__("Print Large Item Codes"), function(){
    ...
    frm.add_custom_button(__("Print Small Item Codes"), function(){
      frappe.ui.form.qz_connect()
      .then(function () {
        var config = qz.configs.create("Godex G500");            // Exact printer name from OS
        var print_data =[];
        ...
        frappe.call({
          ...
          callback: (r) => {
            ...
            //console.log("****Print Item code total_items:", total_items);
            //BA - Barcode, AB - Item code, AA - Item name, AC - Item rate           
            for (var j = 0; j < total_items.length; j = j + 4) {
              ...
              //console.log("****Print Item code before print push");
              print_data.push(
                frm.events.get_EZPL_string_small(frm, group_string)
              );
            }
            return qz.print(config, print_data);
          },
    ...
    // ^Q23,3 // Label height = 15mm (120 dots), gap = 3mm
    // ^W76    // Label width = 150 dots (25mm per label)
    // ^H10   // Print heat/darkness
    // ^P1   // Print count
    // ^S2    // Speed
    // ^AD   // Font set
    // ^C1   // Print direction
    ...
    // ^O0   // Offset 0
    // ^D0    / No delay
    // ^E18     // Enable gap detection with low sensor sensitivity
    // ~R255   // Set max print area in dots
    // ^L      // Start label layout
