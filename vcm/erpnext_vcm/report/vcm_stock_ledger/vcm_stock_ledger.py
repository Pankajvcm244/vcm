...
def execute(filters=None):
	...
	attach_additional_cost_center_info(sl_entries)
...
def get_columns(filters):
	columns = [
		...
		{"label": _("PR Cost Center"), "fieldname": "pr_cost_center", "width": 150},
		{"label": _("SE Cost Center"), "fieldname": "se_cost_center", "width": 150},
		{"label": _("SE VCM Remark"), "fieldname": "vcm_remarks", "width": 150},
	]
...
def attach_additional_cost_center_info(sl_entries):
	# Collect voucher_nos by type to reduce DB hits
	se_vouchers = set()
	pr_vouchers = set()

	for sle in sl_entries:
		if sle.voucher_type == "Stock Entry":
			se_vouchers.add(sle.voucher_no)
		elif sle.voucher_type == "Purchase Receipt":
			pr_vouchers.add(sle.voucher_no)

	# Fetch SE remarks and cache
	se_remarks = {}
	if se_vouchers:
		se_remarks = {
			row.name: row.vcm_remarks
			for row in frappe.get_all(
					"Stock Entry",
					filters={"name": ["in", list(se_vouchers)]},
					fields=["name", "vcm_remarks"]
    		)
}

	# Fetch SE Item cost centers
	se_cost_centers = {}
	if se_vouchers:
		for row in frappe.get_all(
			"Stock Entry Detail",
			filters={"parent": ["in", list(se_vouchers)]},
			fields=["parent", "item_code", "cost_center"]
		):
			se_cost_centers[(row.parent, row.item_code)] = row.cost_center

	# Fetch PR Item cost centers
	pr_cost_centers = {}
	if pr_vouchers:
		for row in frappe.get_all(
			"Purchase Receipt Item",
			filters={"parent": ["in", list(pr_vouchers)]},
			fields=["parent", "item_code", "cost_center"]
		):
			pr_cost_centers[(row.parent, row.item_code)] = row.cost_center

	# Attach values to each entry
	for sle in sl_entries:
		if sle.voucher_type == "Stock Entry":
			sle["se_cost_center"] = se_cost_centers.get((sle.voucher_no, sle.item_code))
			sle["vcm_remarks"] = se_remarks.get(sle.voucher_no)
		elif sle.voucher_type == "Purchase Receipt":
			sle["pr_cost_center"] = pr_cost_centers.get((sle.voucher_no, sle.item_code))
