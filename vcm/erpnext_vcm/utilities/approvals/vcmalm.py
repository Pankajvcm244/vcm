...
from vcm.erpnext_vcm.utilities.whatsapp.powhatsapp import (
...
def get_alm_level(doc, amount_field="total"):
    logging.debug(f"VCM PO in get_alm_level")
    """
    ...
    for l in frappe.db.sql(
        ...
        if eval(f"{deciding_amount} {l.amount_condition}"):
            logging.debug(f"VCM PO in get_alm_level {l}")
            return l
    return None


def assign_and_notify_next_authority(doc, method="Email"):
    logging.debug(f"VCM PO in assign_and_notify_next_authority {doc.workflow_state}")
    user = None
...
def assign_to_next_approving_authority(doc, user):
    logging.debug(f"VCM PO in assign_to_next_approving_authority {user}")
