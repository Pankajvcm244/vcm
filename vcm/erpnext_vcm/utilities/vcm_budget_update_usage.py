...
def update_vcm_pi_budget_usage(pi_doc):
    ...
    if not budget_doc:
        ...
        return False
    budget_updated_flag = True
    total_advance = 0.0  # Initialize advance amount
    if pi_doc.allocate_advances_automatically:        
        ...
        logging.debug(f"in Purchase Invoice Advance: {pi_doc.allocate_advances_automatically}, {total_advance} ")
            ...
            if budget_item.budget_head == pi_doc.budget_head:
                ...
                if PI_FLAG_WITH_PO:  
                    ...
                    if (pi_doc.rounded_total - total_advance ) > budget_item.balance_budget:
                        frappe.throw(f"Budget exceeded for: {pi_doc.budget_head},Balance:{budget_item.balance_budget},Request:{pi_doc.rounded_total}, Adv: {total_advance}")
                        return False
                    else:
                        budget_item.used_budget += (pi_doc.rounded_total - total_advance)  # Update Used Budget
                        budget_item.balance_budget -= (pi_doc.rounded_total -total_advance)  # Adjust Remaining Budget
                        budget_item.unpaid_purchase_invoice += (pi_doc.rounded_total - total_advance)  # Adjust Remaining Budget
                        logging.debug(f"Direct PI W/O PO ,{pi_doc.budget_head}, {budget_item.unpaid_purchase_invoice},{pi_doc.rounded_total},{total_advance}") 
    ...
    for budget_item in budget_doc.get("budget_items") or []:
            ...
            if budget_item.budget_head == pi_doc.budget_head:
                ...
                if PI_FLAG_WITH_PO:  
                    ...
                    budget_item.balance_budget += (pi_doc.rounded_total - total_advance)  # Adjust Remaining Budget
                    budget_item.unpaid_purchase_invoice -= (pi_doc.rounded_total -total_advance)  # Adjust Remaining Budget
                    logging.debug(f"Direct PI revert W/O PO , {budget_item.unpaid_purchase_invoice},{pi_doc.rounded_total},{total_advance}")           
                break
    if budget_updated_flag:
        frappe.throw(f"Budget not found for Budget Head: {pi_doc.budget_head}")
        return False
    ...
    for budget_item in budget_doc.get("budget_items") or []:
            if budget_item.budget_head == pe_doc.budget_head:
                ...
                if PAYMENT_FLAG_WITH_PI: 
                    ...
                    if total_paid_amount > budget_item.unpaid_purchase_invoice:
                        frappe.throw(f"Unpaid Purchase Invoice is not available {pe_doc.budget_head}, Balance: {budget_item.unpaid_purchase_invoice}, Request: {total_paid_amount}")
                    ...
                    logging.debug(f"in update_vcm_pE_budget_usage 3 {total_paid_amount},{budget_item.paid_payment_entry}")
                    if total_paid_amount > budget_item.unpaid_purchase_order:
                        frappe.throw(f"Unpaid Purchase Order is not available {pe_doc.budget_head}, Balance: {budget_item.unpaid_purchase_order}, Request: {total_paid_amount}")
                    budget_item.unpaid_purchase_order -= total_paid_amount  #Decrease PO amount
                    budget_item.paid_payment_entry += total_paid_amount
                else:
                    #reduce and check budget for PI without PO
                    ...
                    budget_item.paid_payment_entry += total_paid_amount 
                    logging.debug(f"update_vcm_ w/O PI pi_budget_usage6, {budget_updated_flag}{budget_item.budget_head},{total_paid_amount}")           
                break 
    if budget_updated_flag:
